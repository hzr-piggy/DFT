...
contains
  ...
  subroutine gyrotropic_main
    ...
    integer           :: n, i, j, k, ikpt, if, ierr, loop_x, loop_y, loop_z, &
                         loop_xyz, ifreq, &
                         file_unit
    logical           :: eval_K, eval_C, eval_D, eval_Dw, eval_NOA, eval_K_spn, eval_NOA_spn, eval_DOS
    ...
    if (index(gyrotropic_task, '-korb') > 0) eval_K = .true.
    if (index(gyrotropic_task, '-kspin') > 0) eval_K_spn = .true.
    ...
    if (index(gyrotropic_task, '-noaorb') > 0) eval_NOA = .true.
    if (index(gyrotropic_task, '-noaspin') > 0) eval_NOA_spn = .true.
...
!    if (.not. (eval_K .or. eval_noa)) eval_spn = .false.
    ...
    if ((.not. spinors) .and. (eval_K_spn .or. eval_NOA_spn)) call io_error( &
    ...
    if (eval_K_spn .or. eval_NOA_spn) then
      call get_AA_R
      call get_SS_R
    endif
    ...
    if (on_root) then
      ...
      if (eval_K) write (stdout, '(/,3x,a)') '* K-tensor orbital  --- Eq.3 of TAS17 '
      
      if (eval_K_spn) write (stdout, '(/,3x,a)') '* K-tensor spin  --- Eq.3 of TAS17 '

      if (eval_Dw) write (stdout, '(/,3x,a)') '* Dw-tensor  --- Eq.12 of TAS17 '

      if (eval_C) write (stdout, '(/,3x,a)') '* C-tensor  --- Eq.B6 of TAS17 '

      if (eval_NOA) write (stdout, '(/,3x,a)') '* gamma-tensor of NOA orbital --- Eq.C12 of TAS17 '
        
      if (eval_NOA_spn) write (stdout, '(/,3x,a)') '* gamma-tensor of NOA spin --- Eq.C12 of TAS17 '

      if (transl_inv) then
        if (eval_K .or. eval_K_spn) &
    ...
    do loop_xyz = my_node_id, PRODUCT(gyrotropic_kmesh) - 1, num_nodes
      ...
      call gyrotropic_get_k_list(kpt, kweight, &
                                 ...
                                 eval_K, eval_K_spn, eval_D, eval_Dw, eval_NOA, eval_NOA_spn, eval_C, eval_dos)
    ...
    if (eval_K_spn) &
      call comms_reduce(gyro_K_spn(1, 1, 1), 3*3*nfermi, 'SUM')
    ...
    if (eval_NOA_spn) &
      call comms_reduce(gyro_NOA_spn(1, 1, 1, 1), 3*3*nfermi*gyrotropic_nfreq, 'SUM')
  ...
  subroutine gyrotropic_get_k_list(kpt, kweight, &
    ...
    logical, intent(in) :: eval_K, eval_K_spn, eval_D, eval_Dw, &
                           eval_C, eval_NOA, eval_NOA_spn, eval_dos
    ...
    if (eval_K_spn .or. eval_NOA_spn) allocate (SS(num_wann, num_wann, 3))

    call wham_get_eig_deleig(kpt, eig, del_eig, HH, delHH, UU)

    if (eval_Dw .or. eval_NOA .or. eval_NOA_spn) then
    ...
    do n1 = 1, gyrotropic_num_bands
      ...
      do ifermi = 1, nfermi
        ...
        !
        if ((eval_K_spn .or. eval_NOA_spn) .and. .not. got_spin) then
        ...
        !
        do j = 1, 3
          if (eval_K_spn) gyro_K_spn(:, j, ifermi) = &
    ...
    if (eval_NOA) &
      call gyrotropic_get_NOA_orb_k(kpt, kweight, eig, del_eig, AA, UU, gyro_NOA_orb)

    if (eval_NOA_spn) &
      call gyrotropic_get_NOA_spin_k(kpt, kweight, eig, del_eig, AA, UU, gyro_NOA_spn)
  ...
  subroutine gyrotropic_get_NOA_orb_k(kpt, kweight, eig, del_eig, AA, UU, gyro_NOA_orb)
    !====================================================================!
    !                                                                    !
  ...
  end subroutine gyrotropic_get_NOA_orb_k


  subroutine gyrotropic_get_NOA_spin_k(kpt, kweight, eig, del_eig, AA, UU, gyro_NOA_spn)
    !====================================================================!
    !                                                                    !
    ! Contribution from point k to the real (antisymmetric) part         !
    ! of the natural  complex interband optical conductivity             !
    ...
    !
    real(kind=dp), intent(in)       :: kpt(3), kweight
    real(kind=dp), dimension(:, :, :, :), intent(inout)    :: gyro_NOA_spn
    complex(kind=dp), dimension(:, :, :), intent(in)       :: AA
    complex(kind=dp), dimension(:, :), intent(in)       :: UU
    real(kind=dp), dimension(:), intent(in)       :: eig
    real(kind=dp), dimension(:, :), intent(in)       :: del_eig
...
!      call gyrotropic_get_NOA_Bnl_orb(eig, del_eig, AA, num_occ, occ_list, num_unocc, unocc_list, Bnl_orb)
  ...
  end subroutine gyrotropic_get_NOA_spin_k
