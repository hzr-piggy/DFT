...
class w90(object):
    ...
    *prefix*\_centres.dat) one needs to include the following flags in the win
    ...
    def model(self,zero_energy=0.0,min_hopping_norm=None,max_distance=None,ignorable_imaginary_part=None,nspin=1):
        """
...
##########################################################################################################
            ...
            # make the model object
            tb=tb_model(3,3,self.lat,self.red_cen[0::2],nspin=2)
            num_wan=self.num_wan//2

            # remember that this model was computed from w90
            tb._assume_position_operator_diagonal=False

            # add onsite energies as 2by2 matrix
            for i in range(num_wan):
                tmp_ham=self.ham_r[(0,0,0)]["h"][i*2:i*2+2,i*2:i*2+2]/float(self.ham_r[(0,0,0)]["deg"])
                onsite=tmp_ham.real
                if np.sum(np.abs(tmp_ham.imag))>4.0E-9:
                    raise Exception("Onsite terms should be real!")
                tb.set_onsite(onsite-zero_energy*np.eye(2),i)
            ...
            for R in self.ham_r:
                ...
                if use_this_R==True:
                    for i in range(num_wan):
                        vec_i=self.xyz_cen[i*2]
                        for j in range(num_wan):
                            vec_j=self.xyz_cen[j*2]
                            # get distance between orbitals
                            dist_ijR=np.sqrt(np.dot(-vec_i+vec_j+vecR,
                                                    -vec_i+vec_j+vecR))
                            # to prevent double counting
                            if not (avoid_diagonal==True and j<=i):
                                ...
                                # divide the matrix element from w90 with the degeneracy
                                tmp_ham=self.ham_r[R]["h"][i*2:i*2+2,j*2:j*2+2]/float(self.ham_r[R]["deg"])
                                
                                # only if big enough matrix element
                                if min_hopping_norm is not None:
                                    test_hopping=(np.abs(tmp_ham)>min_hopping_norm).astype(int)
                                    if np.sum(test_hopping)==0:
                                        continue
                                    else:
                                        tmp_ham=np.multiply(tmp_ham,test_hopping)

                                # remove imaginary part if needed
                                if ignorable_imaginary_part is not None:
                                    for ii in [0,1]:
                                        for jj in [0,1]:
                                            if np.abs(tmp_ham[ii,jj].imag)<ignorable_imaginary_part:
                                                tmp_ham[ii,jj]=tmp_ham[ii,jj].real+0.0j
